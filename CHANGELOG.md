...
- **The party row a message box cuts through keeps its colours**
  (Gen1Party 1.8.2). 1.25.0 dropped an icon's matte and its true-colour mark
  ...
  stayed in colour. Only the covered part lets go now. Same cut Gen1ModernBag
  makes sideways in 1.13.1, turned ninety degrees: a box edge here is a
  horizontal, so it clips the height.

- **`BUY` and `SELL` name themselves in the header.** Same empty box the item
  PC had, and the same cause — `ShopMenu` opens both lists with
  ...
  the bag being reordered. There is nothing to reorder in a shop's stock.
  Behaviour rather than content, because a row's `price` would have said the
  same thing right up until an empty sell list came up with only its `CANCEL`
  row on it.
...
- **The pale box with the black ring, round the character on the way into a
  ...
  square — and this theme paints its one-pixel ring round the edge of it — the
  black outline.
